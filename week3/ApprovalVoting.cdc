...
pub contract ApprovalVoting {
    ...
    pub resource Ballot {
        ...
        pub fun vote(proposal: Int, token: &AnyResource{FungibleToken.Balance}) {
            pre {
                self.proposals[proposal] != nil: "Cannot vote for a proposal that doesn't exist"
            }
            self.choices[proposal] = Int(token.balance)
        }
    }
    ...
    pub fun cast(ballot: @Ballot) {
        ...
        while index < self.proposals.length {
            if ballot.choices[index] != 0 {
                // tally the vote if it is approved
                self.votes[index] = self.votes[index]! + ballot.choices[index]!
            }
            index = index + 1;
        }
